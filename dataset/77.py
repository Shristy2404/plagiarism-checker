...
def polynomial_lasso_regressor(deg, X_train, X_test, y_train, y_test) :
    ...
    if deg == 10 :
        ...
        lasso_train_score = lasso.score(X_poly, y_train)
        lasso_test_score = lasso.score(X_test_poly, y_test)
        print(' Poly linear_train_score ', linear_train_score)
        print(' Poly linear_test_score ', linear_test_score)
        print(' Poly lasso_train_score ', lasso_train_score)
        print(' Poly lasso_test_score ', lasso_test_score)
...
def main() :
    ...
    plt.xlabel(" DEGREES ")
    plt.ylabel(" ERMS ")
    plt.savefig('lasso_deg_rmse.png')
    plt.show()
